...
import { SHADER_PREAMBLE, INITIAL_USER_CODE } from "./shaders";
...
async function main() {
    ...
    try {
        ...
        // Initialize Editor
        const editor = new Editor(editorContainer, INITIAL_USER_CODE);

        // Initial compilation
        const compile = async (code: string) => {
            try {
                const pipeline = await pipelineManager.createPipeline(SHADER_PREAMBLE + code, [globalUniforms.bindGroupLayout]);
        ...
        await compile(INITIAL_USER_CODE);
